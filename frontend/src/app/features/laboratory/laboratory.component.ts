...
@Component({
  ...
  template: `<section class="page"><div class="hero lab-head"><span class="pill">Laboratorio interactivo</span><h1>Experimenta y observa el agente RL</h1><p>Configura entrenamientos, guarda resultados comparables o ejecuta la simulación en vivo para ver cómo cada evento produce una respuesta defensiva.</p></div><div class="lab-tabs" role="tablist" aria-label="Vistas del laboratorio"><button type="button" role="tab" [attr.aria-selected]="activeTab==='experiment'" [class.active]="activeTab==='experiment'" (click)="activeTab='experiment'">Experimento</button><button type="button" role="tab" [attr.aria-selected]="activeTab==='live'" [class.active]="activeTab==='live'" (click)="activeTab='live'">Simulación en vivo</button></div>@if(activeTab==='experiment'){<app-experiments/>}@else{ @if(error){<p class="notice error panel">{{error}}</p>}<app-live-simulation-view [state]="state" (stateChange)="syncState($event)"/>}</section>`,
